		...
		for k, v in attrs.items():
			if isinstance(v, Field):
				...
				if v.primary_key:
					...
					fields.append(k)
		if not primary_key:
			raise StandardError('Primay key not found.')
...
class Model(dict, metaclass=ModelMetaclass):
	...
	async def findAll(cls, where=None, args=None, **kw):
		...
		if args is None:
			args = []
		orderBy = kw.getattr('orderBy', None)
		if orderBy:
			sql.append('order by')
			sql.append(orderBy)
	...
	async def save(self):
		...
		if rows != 1:
			loggin.warn('failed to inset record: affected rows: %s' % rows)
